...
summary: このDiffは、Azure AI Searchのドキュメントに関する重要な更新を示しています。新しい画像ファイルが追加され、統合ベクトル化に関する詳細なガイドが新たに提供されたことで、情報の理解が向上しました。また、目次に新しいリンクが追加され、情報へのアクセスが容易になっています。さらに、インデクサーのサポートするデータソースに`OneLake`が追加され、より多様なデータソースに対応できるようになりました。全体として、これらの更新はAzure
  AI Searchドキュメントの質とユーザーエクスペリエンスを向上させる重要な改善です。
...
# Highlights
本Diffは、Azure AI Searchのドキュメントに関連するいくつかの重要な更新を示しています。新しい画像およびドキュメントの追加に加え、既存ドキュメントのマイナーな更新や目次の改善が含まれています。

## 新しい機能
- 新しい画像ファイル`data-lake-storage-account.png`が追加され、関連するドキュメントを視覚的に補完します。
- `search-how-to-integrated-vectorization.md`という新しいドキュメントが追加され、統合ベクトル化の使用方法に関する詳細なガイドが提供されます。
- 目次(`toc.yml`)に「統合ベクトル化の使用」へのリンクが追加され、情報へのアクセス性が向上しました。

## 破壊的な変更
この差分には破壊的な変更は含まれていません。

## その他の更新
- `search-indexer-overview.md`の修正には、インデクサーがサポートするデータソースリストへの`OneLake`の追加が含まれています。これにより、新しいデータソースタイプをインデクシング可能としました。

# Insights
このDiffでは、Azure AI Searchドキュメントへの貢献が3つの重要な方向で進化しています。まず視覚的なサポートとして新しい画像が追加され、ドキュメントの理解を進めるための補助役割を果たします。次に、統合ベクトル化に関する新しいドキュメントが提供され、ユーザーが具体的な設定やプロセスの流れを理解する助けとなります。特に、REST APIを使用して、どのようにデータを準備し、スキルセットを作成し、データソースからのベクトル化を行うかが詳しく説明されています。

また、目次の改善により、ユーザーは求めている情報に迅速にアクセスでき、ナビゲーション効率が向上しました。特筆すべきは、検索インデクサーの概要における更新で、`OneLake`が新しくサポートされるデータソースに加わったことです。これはAzure AI Searchが対応するデータソースの範囲を広げ、さらに多様なニーズに応えられるようになることを示しています。

トータルでこれらの更新は、Azure AI Searchドキュメントの包括性とユーザーエクスペリエンスを高める重要なステップであり、今後の実装や利用においてより豊富な情報を提供します。

# Summary Table
|  Filename  | Type |    Title    | Status | A  | D  | M  |
|------------|------|-------------|--------|----|----|----|
| [data-lake-storage-account.png](#item-42af4a) | new feature | 新しい画像の追加: data-lake-storage-account.png | added | 0 | 0 | 0 | 
| [search-how-to-integrated-vectorization.md](#item-86fb1e) | new feature | 新しいドキュメントの追加: search-how-to-integrated-vectorization.md | added | 891 | 0 | 891 | 
| [search-indexer-overview.md](#item-292796) | minor update | 検索インデクサーの概要の更新 | modified | 2 | 1 | 3 | 
| [toc.yml](#item-c4768f) | minor update | トピックの目次に「統合ベクトル化の使用」を追加 | modified | 2 | 0 | 2 | 
...
```json
{
    ...
    "modification_title": "新しい画像の追加: data-lake-storage-account.png"
}
```

### Explanation
この変更は、`articles/search/media/search-how-to-integrated-vectorization/`ディレクトリに新しい画像ファイル`data-lake-storage-account.png`を追加したことを示しています。この画像は、ビジュアルコンテンツを提供するために使われ、検索機能に関連するドキュメントを補完する役割を果たします。現在、変更内容は追加されたものであり、削除された要素や変更はありません。新しい画像は、[[こちらのリンク]](https://github.com/MicrosoftDocs/azure-ai-docs/blob/8fb62862c4cf33775348c35025dbf207fd0f65e4/articles%2Fsearch%2Fmedia%2Fsearch-how-to-integrated-vectorization%2Fdata-lake-storage-account.png)からアクセスできます。
...
{
    "modification_type": "new feature",
    "modification_title": "新しいドキュメントの追加: search-how-to-integrated-vectorization.md"
}
```

### Explanation
この変更は、新しいMarkdownファイル`search-how-to-integrated-vectorization.md`を`articles/search/`ディレクトリに追加したことを示しています。このファイルには、Azure AI Searchにおける統合ベクトル化を使用する方法についての詳細な説明が含まれています。具体的には、REST APIを使用して、スキルセットを設定し、データソースからのコンテンツのチャンク化とベクトル化を行うプロセスについて解説しています。

ドキュメントは891行の内容で構成されており、主要なトピックには次のような内容が含まれます：

- 統合ベクトル化の設定方法や前提条件
- サポートされているデータソースおよび埋め込みモデルの説明
- ロールベースのアクセス制御や接続情報の確認方法
- データの準備とスキルセットの作成方法
- ベクトルインデックスの作成方法およびクエリの確認方法

この新しいドキュメントは、ユーザーが統合ベクトル化を実装する際の参考資料となります。詳細は[[こちらのリンク]](https://github.com/MicrosoftDocs/azure-ai-docs/blob/8fb62862c4cf33775348c35025dbf207fd0f65e4/articles%2Fsearch%2Fsearch-how-to-integrated-vectorization.md)から入手可能です。
...
```json
{
    ...
    "modification_title": "検索インデクサーの概要の更新"
}
```

### Explanation
この変更は、`search-indexer-overview.md`ファイルの修正を示しており、主にメタデータと内容の一部が更新されています。具体的には、以下の変更が行われました：

1. **日付の変更**: `ms.date`フィールドが`04/09/2025`から`04/18/2025`に更新され、新しい日付が反映されました。これは、ドキュメントの最新性を保つための重要な修正です。

2. **新しい項目の追加**: インデクサーがサポートするデータソースリストに、`OneLake`が追加されました。これにより、ユーザーはOneLakeに対してもインデクシングを行うことができるようになります。

3. **内容の微調整**: 追加された新しい項目は、OneLakeに関するものであり、「プレビュー中」との注記が付けられています。この変更は、ドキュメントの正確さと包括性を向上させるものです。

このファイルは、Azure AI Search内でのインデクサーの機能と利用可能なデータソースについての概観を提供しており、ユーザーにとって重要な情報を含んでいます。更新された内容の詳細は[[こちらのリンク]](https://github.com/MicrosoftDocs/azure-ai-docs/blob/8fb62862c4cf33775348c35025dbf207fd0f65e4/articles%2Fsearch%2Fsearch-indexer-overview.md)から確認できます。
...
{
    "modification_type": "minor update",
    "modification_title": "トピックの目次に「統合ベクトル化の使用」を追加"
}
```

### Explanation
この変更は、`toc.yml`ファイルに対して行われた修正を示しています。具体的には、以下の内容が追加されました：

1. **新しい項目の追加**: 「Use integrated vectorization（統合ベクトル化の使用）」という新しい項目が目次に追加されました。この項目は、`search-how-to-integrated-vectorization.md`というドキュメントへのリンクを提供しています。この更新により、ユーザーが統合ベクトル化の利用方法に関する情報を簡単に見つけられるようになります。

2. **全体の構造の改善**: 新しい項目が追加されたことにより、目次がさらに充実し、関連する情報へのナビゲーションが容易になります。これにより、ユーザー体験が向上し、必要な情報に迅速にアクセスできるようになります。

更新された目次の詳細は[[こちらのリンク]](https://github.com/MicrosoftDocs/azure-ai-docs/blob/8fb62862c4cf33775348c35025dbf207fd0f65e4/articles%2Fsearch%2Ftoc.yml)から確認できます。
