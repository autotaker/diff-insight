...
summary: |-
  ...
  新しいチュートリアルファイルが追加され、複数の Blob インデクサを1つの検索インデックスに構成する方法が解説されています。破壊的変更はなく、小規模な更新としてエラーメッセージの削除や目次項目の追加が行われました。この変更は、ユーザーエクスペリエンスを向上させ、特に開発者がAzure AI Searchの柔軟性を活かしてプロジェクトを実装する際に役立つ情報源となることを目的としています。また、エラーメッセージの削除はトラブルシューティングを簡素化し、目次の更新はドキュメントのナビゲーションを改善することに寄与しています。これにより、ユーザーが求める情報に迅速にアクセスできるようになり、全体的な使いやすさが向上しています。
...
- **新機能:** 複数の Blob インデクサを1つの検索インデックスに構成する方法を解説した新しいチュートリアルファイルが追加されました。
- **破壊的変更:** なし
- **その他の更新:** エラーメッセージの削除や目次の追加などの小規模な更新。

## 新機能
- `search-how-to-multiple-indexers-one-index.md` ファイルが新規追加され、Azure AI Search における複数の Blob インデクサを1つのインデックスに構成するチュートリアルが追加されました。このチュートリアルは、複数のファイル形式（例: DOCX, JSON, CSV）を対象とし、統合スキーマインデックスへのデータ書き込み方法を詳述しています。

## 破壊的変更
なし。

## その他の更新
- `search-how-to-index-sharepoint-online.md` ファイルから、エラーメッセージに関する情報が削除され、ユーザーがより直接的に操作ガイドを理解できるよう調整されました。
- `toc.yml` ファイルに目次項目を追加し、新しいBlobインデクサ構成チュートリアルへのアクセスを容易にしました。

# インサイト

この更新はドキュメントにおけるユーザーエクスペリエンスを向上させるためのものです。特に、新しいチュートリアルの追加は、Azure AI Search の柔軟性を活かしたプロジェクトを実装しようとする開発者にとって貴重な情報源となるでしょう。複数形式のファイルを効率的にインデックス化し、統合することで、一つの検索インデックス内で多様なデータを取り扱えるメリットがあります。

エラーメッセージの削除も、ユーザーが操作を進めやすくするための意図と考えられます。エラーが発生した場合に、指定されたエラーメッセージの詳細情報が不要とされたことで、シンプルで効率の良いトラブルシューティングが可能になるかもしれません。

最終的に、目次への新しい項目の追加はドキュメント全体のナビゲーション改善に寄与し、ユーザーが探している情報に迅速にアクセスできるようにしています。これにより、ユーザーが自己解決できる範囲が広がり、サポートの手間も軽減される可能性があります。これらの改訂は、ドキュメントの使いやすさと情報の正確性を高めることに繋がる重要なステップです。

# Summary Table
|  Filename  | Type |    Title    | Status | A  | D  | M  |
|------------|------|-------------|--------|----|----|----|
| [search-how-to-index-sharepoint-online.md](#item-8c099c) | minor update | SharePoint Online のインデックス作成に関するエラーメッセージの削除 | modified | 0 | 7 | 7 | 
| [search-how-to-multiple-indexers-one-index.md](#item-5ccefd) | new feature | 1 つの検索インデックス用の複数の Blob インデクサを構成するチュートリアル | added | 1151 | 0 | 1151 | 
| [toc.yml](#item-c4768f) | minor update | 目次に複数の Blob インデクサを構成する項目の追加 | modified | 2 | 0 | 2 | 
...
```json
{
    ...
    "modification_title": "SharePoint Online のインデックス作成に関するエラーメッセージの削除"
}
```

### Explanation
このコードの変更では、`search-how-to-index-sharepoint-online.md` ファイルの内容が修正され、特定のエラーメッセージに関する情報が削除されました。具体的には、SharePoint サイト上のファイルに暗号化が有効な場合に表示されるエラーメッセージとその説明が削除されています。この変更により、ユーザーはエラー情報に関する誤解や混乱を避けることができ、よりシンプルな指示を受け取ることができます。また、関連するコンテンツセクションはそのまま残されているため、ユーザーは関連情報へのアクセスは引き続き可能です。
...
```json
{
    ...
    "modification_title": "1 つの検索インデックス用の複数の Blob インデクサを構成するチュートリアル"
}
```

### Explanation
このコードの変更では、新しいファイル `search-how-to-multiple-indexers-one-index.md` が追加され、Azure AI Search を使用して複数の Blob インデクサを1つのインデックスに構成する方法についての詳細なチュートリアルが提供されています。このチュートリアルでは、DOCX、JSON、CSV などの異なる形式のコンテンツを対象とした3つの独立した Blob インデクサを設定し、これらがどのようにして統合スキーマの Azure AI Search インデックスにデータを書き込むかが説明されています。

チュートリアルの中では、必要な準備、インデックスとデータソースの作成、インデクサの設定、各種パイプラインの構成方法が網羅されており、最終的には、複数のインデクサから得られる結果をすべて表示する方法まで解説されています。また、関連する要件や注意事項、トラブルシューティングのガイドも含まれており、全体的に新しい機能の利用に必要な情報が一通り揃っています。
...
+        - name: Configure multiple blob indexers
+          href: search-how-to-multiple-indexers-one-index.md
...
{
    "modification_type": "minor update",
    "modification_title": "目次に複数の Blob インデクサを構成する項目の追加"
}
```

### Explanation
このコードの変更では、`toc.yml`ファイルが修正され、目次に「複数の Blob インデクサを構成する」という新しい項目が追加されました。この変更により、ユーザーは Azure AI Search において複数の Blob インデクサを設定する方法を学ぶためのチュートリアルに簡単にアクセスできるようになります。具体的には、この項目のリンクが追加されることで、他のチュートリアルやリソースと併せて、ユーザーがコンテンツを効率的に探しやすくなっています。全体として、この変更は、ドキュメントのナビゲーションを改善し、重要な情報へのアクセスを促進します。
