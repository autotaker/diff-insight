---
date: '2026-02-12'
permalink: https://github.com/MicrosoftDocs/azure-ai-docs/compare/MicrosoftDocs:8eb0165...MicrosoftDocs:e4d4d10
summary: このコード差分では、AzureのCognitive Searchに関するドキュメントがマイナーな更新を受け、主に日付の変更やAPIバージョンの更新が行われました。新機能としてセンシティビティラベルに関する既知の問題が追加され、ユーザーの注意が喚起されています。破壊的変更はなく、ドキュメントの日付の更新や整理、APIバージョンの更新などが行われ、重要な注意事項が削除されることで文書が簡略化されました。これにより、ユーザーは最新の情報にアクセスしやすくなり、特にAzure
  AI Searchの機能を活用する際に有益です。
title: Diff Insight Report - search

---

[View Diff on GitHub](https://github.com/MicrosoftDocs/azure-ai-docs/compare/MicrosoftDocs:8eb0165...MicrosoftDocs:e4d4d10){target="_blank"}

<format>
# ハイライト
このコード差分では、AzureのCognitive Searchおよび関連ドキュメントに対してマイナーな更新が行われ、主に日付の変更とAPIバージョンの更新が焦点です。新しい機能の追加や既知の問題の追記、重要な注意事項の削除が含まれます。

## 新機能
- センシティビティラベルに関する既知の問題が新たに追加され、ユーザーの注意を喚起しています。
...
## その他の更新
- ドキュメントの日付の更新。
- 一部のセクションが整理され、最新の情報に基づく更新。
- APIバージョンが「2025-05-01-preview」から「2025-11-01」に更新。
- 重要な注意事項の削除とそれに伴うドキュメントの簡略化。

# インサイト
このコード差分は、主にAzureにおける検索機能に関連するドキュメントの維持管理を目的としています。最新の日付やAPIバージョンに更新されることで、ユーザーは常に正確かつ最新の情報にアクセスできるようになります。例えば、セクションの削除や注意事項の整理は、ユーザーが不必要な混乱を避け、最も関連性の高い情報に集中できるように設計されています。

特に、新しく追加されたセンシティビティラベルに関する既知の問題は、これらのラベルが適用された環境での具体的な動作や制約を示し、ユーザーのシステム安定性や運用面でのトラブル回避に役立ちます。一方、重要な注意事項の削除は、特定の機能をテストする際の制限を緩和し、ユーザーへの実用性を向上させています。

また、新機能に関する情報や改善点がより明確に示されており、特にAzure AI Searchの最新の機能を活用したいユーザーにとって有益です。これらの更新は、Azureのドキュメントがユーザーフレンドリーであり、技術的変化に迅速に対応していることを示しています。
</format>

# Summary Table
|  Filename  | Type |    Title    | Status | A  | D  | M  |
|------------|------|-------------|--------|----|----|----|
| [cognitive-search-skill-content-understanding.md](#item-c7787e) | minor update | Cognitive Search Skill Content Understandingの更新日時とAPIバージョンの修正 | modified | 2 | 2 | 4 | 
| [search-how-to-index-sharepoint-online.md](#item-8c099c) | minor update | SharePoint Onlineのインデクシング手順における日付の更新と重要な注意事項の削除 | modified | 1 | 3 | 4 | 
| [search-indexer-sensitivity-labels.md](#item-2a7bfc) | minor update | センシティビティラベルに関するドキュメントの日付更新と既知の問題の追加 | modified | 1 | 2 | 3 | 
| [search-query-sensitivity-labels.md](#item-3e1f8a) | minor update | センシティビティラベルのクエリ時強制に関するドキュメントの日付更新とセクションの削除 | modified | 1 | 2 | 3 | 
| [whats-new.md](#item-fa71b4) | minor update | 2025年の新機能に関する文書の更新と変更 | modified | 68 | 165 | 233 | 
...
```json
{
    ...
    "modification_title": "Cognitive Search Skill Content Understandingの更新日時とAPIバージョンの修正"
}
```

### Explanation
この変更は、AzureのCognitive Search Skill Content Understandingに関するドキュメントに対するマイナーな更新です。具体的には、文書内の日付が2025年11月10日から2026年2月11日に変更され、呼び出すREST APIのバージョンが「2025-05-01-preview」から「2025-11-01」にアップデートされています。この変更により、APIの正確なバージョンとリリース日が最新の情報に基づいて更新され、ユーザーが新しい仕様に正確にアクセスできるようになっています。
...
{
    "modification_type": "minor update",
    "modification_title": "SharePoint Onlineのインデクシング手順における日付の更新と重要な注意事項の削除"
}
```

### Explanation
この変更は、SharePoint Onlineのインデクシング手順に関するドキュメントに対するマイナーな更新です。具体的には、ドキュメント内の日付が2025年11月9日から2026年1月28日に変更されました。また、重要な注意事項の一部が削除されています。この注意事項は、SharePoint ACLのプレビュー機能を同じインデクサーやインデックスでテストまたは有効にしないように警告していました。これにより、文書が最新の日付を反映するだけでなく、特定の制限が緩和されたことを示しています。ユーザーにとって重要な情報を簡潔に伝えるための更新と言えます。
...
+ms.date: 01/28/2026  
 ...
 ms.author: gimondra  
 ---
@@ -55,7 +55,6 @@ These updates are detected if they occurred since the last indexer run.
...
{
    "modification_type": "minor update",
    "modification_title": "センシティビティラベルに関するドキュメントの日付更新と既知の問題の追加"
}
```

### Explanation
この変更は、Azure AI Searchにおけるセンシティビティラベルのインデクサー設定に関するドキュメントのマイナーな更新です。具体的には、文書の日付が2025年11月20日から2026年1月28日に変更されました。また、センシティビティラベルが有効なインデックスに関する既知の問題が新たに追加されています。この問題は、インデックスがドキュメントを列挙できず、その結果、ソフト削除操作が正常に実行されないことを示しています。さらに、センシティビティラベルを利用したインデックスでは、補完や提案APIが無効であり、ゲストアカウントやクロステナントクエリがサポートされないことも記載されています。これにより、ユーザーに対して重要な制限事項が明確に伝えられています。
...
{
    "modification_type": "minor update",
    "modification_title": "センシティビティラベルのクエリ時強制に関するドキュメントの日付更新とセクションの削除"
}
```

### Explanation
この変更は、Azure AI Searchにおけるセンシティビティラベルのクエリ時強制に関するドキュメントのマイナーな更新です。ドキュメントの日付が2025年11月18日から2026年1月28日に変更されました。さらに、センシティビティラベルの機能とACLベースのセキュリティフィルターの併用についての警告セクションが削除されました。この削除により、センシティビティラベルとACLの併用がサポートされていないことについての明示的な指示がなくなり、従来の情報が簡潔に整理された形となっています。この更新は、文書の現行状態を最新の情報に保ち、ユーザーにとって重要な情報の明確さを向上させています。
...
+ms.date: 02/11/2026
 ...
 ms.topic: overview
 ms.custom:
@@ -26,176 +26,79 @@ Learn about the latest updates to Azure AI Search functionality, docs, and sampl
 |--|--|
...
+| September | General availability | [Logic app workflow integration](search-how-to-index-logic-apps.md). |
+| September | General availability | [OneLake indexer](search-how-to-index-onelake-files.md). |
+| September | General availability | [Document Layout skill](cognitive-search-skill-document-intelligence-layout.md). |
...
{
    "modification_type": "minor update",
    "modification_title": "2025年の新機能に関する文書の更新と変更"
}
```

### Explanation
この変更は、Azure AI Searchにおける2025年の新機能に関する文書のマイナーな更新です。特に、文書の日付が2026年2月2日から2026年2月11日に変更され、また文書内に新たに68の追加行があり、165行が削除されています。これは、2025年の新機能についての情報を整理・更新し、ユーザーに対してより明確な情報を提供することを目的としています。

具体的には、新しい機能や改善点、REST APIのバージョンに関する情報が更新され、ポータルのサポートやエージェンティック検索の詳細が強調されています。削除されたセクションは以前のバージョンの情報であるため、最新の情報に基づいて文書が整理されています。この更新により、ユーザーはAzure AI Searchの新機能をより簡単に理解できるようになっています。
